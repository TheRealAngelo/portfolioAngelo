...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const btnDown = document.getElementById('btn-down');
    const btnHome = document.getElementById('btn-home');
    const sections = ['login', 'about', 'resume', 'certification', 'projects', 'contact'];
    ...
    function updateButtonVisibility() {
        ...
        if (btnUp) {
            if (currentIndex > 0) {
                ...
                if (hasReachedAbout && currentIndex === 1 && sections[currentIndex - 1] === 'login') {
    ...
    function preventScrollToLogin() {
        if (!hasReachedAbout || isScrollingProgrammatically || !aboutSection) return;
    ...
    function resetScrollRestriction() {
        ...
        isScrollingProgrammatically = false;
        console.log('Scroll restrictions reset - user can access all sections again');
    }
    ...
    window.addEventListener('touchmove', handleRestrictedScroll, { passive: false });
    ...
    const navLinks = document.querySelectorAll('nav a[href^="#"], .nav-mobile-links a[href^="#"]');
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            const targetId = this.getAttribute('href').substring(1);
